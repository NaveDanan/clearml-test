...
# from utils.data_prep2 import data_prep, print_system_usage  # Ensure this import matches your file structure
# from utils.general import increment_path
# from utils.resnet_metrics import ConfusionMatrix
# from utils.plots import plot_results, plot_pr_curve, plot_confusion_matrix  # Import necessary functions

# Test for resnet with CA block
# from resnet_ca.model_with_dropout import resnet50_ca
import clearml
...
task = Task.init(project_name='new-ResNet-pytorch', task_name='Training', output_uri=True)
...
def get_augment_transform(multiplier, aug_params):
    ...
    return transforms.Compose([
        transforms.RandomHorizontalFlip() if aug_params.get('RandomHorizontalFlip') else nn.Identity(),
        transforms.RandomRotation(aug_params.get('RandomRotation', 0)),
        transforms.ColorJitter(**aug_params.get('ColorJitter', {})),
        transforms.RandomAffine(**aug_params.get('RandomAffine', {})),
        transforms.Lambda(lambda x: [x] * multiplier)
    ])
...
def train_model(args, scheduler, model, train_loader, val_loader, criterion, optimizer, device='cpu'):
    ...
    scaler = GradScaler() # # Initialize GardScaler for mixed precision training
...
if __name__ == '__main__':
    ...
    state_dict = torch.load("/content/resnet152-b121ed2d.pth", weights_only=False)
    ...
    scheduler_params = {'mode': 'min', 'factor': 0.01, 'patience': 3, 'verbose': True}
